...
| 경로 | 화면 | 무엇이 도는가 |
| --- | --- | --- |
| `/` (`index.html`) | **인천이니?** 챗봇 화면 | "강제 코스"는 아래 엔진을 그대로 호출한다. 식사 동행만 아직 가상 인물 데모 |
| `/curator.html` | **지금갈래?** 코스 큐레이터 | 같은 엔진의 폼 기반 화면. 이동수단·현장 메모까지 입력할 수 있다 |

두 화면 모두 `POST /api/recommend`를 부르고, 실패하면 브라우저에서 같은 `lib/recommend.js`를
실행합니다. 챗 화면은 대화 흐름을 위해 이동수단을 묻지 않고 대중교통으로 고정하며, 목적은
"안체인 맛집"에서 고른 분위기(혼밥·걷기·조용한 카페·사진)를 그대로 실어 보냅니다.
